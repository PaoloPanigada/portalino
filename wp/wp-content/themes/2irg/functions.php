...
//	function tied to the excerpt_more filter hook.
	...
	function d5businessline_content() { the_content('<span class="read-more">Read More...</span>'); }
//	D5 BusinessLine and WordPress Credit
	function d5businessline_credit() {
		echo '&nbsp;| Powered by: <a href="mailto:2irg.migrazione@lutech.it">2irg.migrazione@lutech.it</a>';
	}
...
add_action('admin_init', 'remove_comments_menu');

function remove_uploads_menu() {
    remove_menu_page('upload.php');
}
//add_action('admin_init', 'remove_uploads_menu');
...
add_action('admin_head', 'custom_bar');

function custom_login_logo() {
	echo "<style type='text/css'>
	h1 a { background-image: url('http://10.79.225.252/support2i/wp-content/uploads/2015/06/logo_login.png') !important; }
	.login h1 a { margin: 0 !important; background-size: 250px auto; width: 300px; height: 60px !important;}
	</style>";
}
add_action('login_head', 'custom_login_logo');

/*$ref = $_SERVER['HTTP_REFERER'];
if($ref == 'http://10.79.225.252/support2i/consuntivazioni/') {
	function admin_default_page() {
		return '/support2i/consuntivazioni';
	}
